...
def on_message(client, userdata, msg):
    ...
    if msg.topic == 'particles':
        ...
        with open(f"{example_dir}/{PARTICLE_TYPE}.json") as f:
            import json
            PARTICLE_EFFECT_DATA = json.load(f)
        PARTICLE_EFFECT = ParticleEffect.load_from_dict(PARTICLE_EFFECT_DATA)
        ...
        if int(msg.payload.decode()) <= 30:
            ...
            if PARTICLE_POSITION >= 0 and PARTICLE_POSITION <= 10:
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['red'] = [55, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['green'] = [55, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['blue'] = [255, 255]
            elif PARTICLE_POSITION > 10 and PARTICLE_POSITION <= 20:
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['red'] = [55, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['green'] = [255, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['blue'] = [55, 255]
            elif PARTICLE_POSITION > 20 and PARTICLE_POSITION <= 30:
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['red'] = [255, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['green'] = [55, 255]
                PARTICLE_EFFECT_DATA['emitters'][0]['particle_settings']['blue'] = [55, 255]
        else:
            pass
